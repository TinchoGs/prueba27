...
nombres_seleccionados = st.multiselect("Seleccione uno o más clientes:", nombres_clientes)

# Filtrar el DataFrame según la selección
if nombres_seleccionados:
    poolresultado = df_clientes[df_clientes['Cliente'].isin(nombres_seleccionados)]
    
    if not poolresultado.empty:
        # Mostrar solo la columna RazonSocial
        st.write("Clientes encontrados:")
        st.dataframe(poolresultado[['Cliente']])
    else:
        st.write("No se encontraron clientes con los nombres seleccionados.")
else:
    st.write("Por favor, seleccione al menos un cliente.")
...
tiempo_cotizacion = st.selectbox("selecicone la cantidad de dias", ValidezCot)
...
if tipo_venta == 'Venta por unidad':
    if st.button("Calcular"):
        ...
        data = [
                ['Codigo', 'Articulo', 'Precio/USD', 'Cantidad', 'Total']
            ]

   
        for index, row in cotiza_df.iterrows():
    # Asegúrate de que estás accediendo a los valores correctamente
            data.append([row["Codigo"], row["Articulo"], row["Precio/USD"], row["Cantidad"],  row["Total"]])
...
# Función para dibujar en el PDF
    ...
    # Título y pretexto
        ...
        st.download_button(
            ...
            data=pdf_buffer,
            file_name="Cotizacion " + Vendedor + ".pdf",
    ...
    # Asegúrate de que estás accediendo a los valores correctamente
        ...
        tablo = Table(data)
        para = Paragraph("Título de la Página")
    ...
    # Estilo de la tabla
        ...
        cliente_data = [['RazonSocial']]  # Encabezado

        # Asegúrate de que estás accediendo a los valores correctamente
        for index, row in poolresultado.iterrows():
            cliente_data.append([row["Cliente"]])
...
# Función para dibujar en el PDF
    ...
    # Título y pretexto
            ...
            c.drawString(1.8 * inch, height - 1.2 * inch, tiempo_cotizacion)
    ...
    # Dibujar la tabla de información de contacto
        ...
        elementos = [para]  # Solo el párrafo, la imagen se dibuja en la función draw
